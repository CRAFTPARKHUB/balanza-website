var t, e = Object.create,
    ...
    l = (t, n, i) => (i = null != t ? e(s(t)) : {}, u( // If the importer is in node compatibility mode or this is not an ESM
    ...
    g = class {
        ...
        constructor(t) {
            this.bytes = t, this.offset = 0, this.view = d(this.bytes), this.decoder = new TextDecoder;
        }
    };
...
var y = "undefined" != typeof window,
    v = y && "function" == typeof window.requestIdleCallback; // src/code-generation/components/cms/bundled/assert.ts
function w(t, ...e) {
...
function E(t, e, r, n) {
    w(t >= e, t, "outside lower bound for", n), w(t <= r, t, "outside upper bound for", n);
}
var M = class {
    getOffset() {
        return this.offset;
    ...
    constructor() {
        this.offset = 0, this.bytes = new Uint8Array(I), this.view = d(this.bytes), this.encoder = new TextEncoder, this.encodedStrings = /* @__PURE__ */ new Map;
    }
...
var N = class t {
    static fromString(e) {
        ...
        return w(F(r), "Invalid chunkId"), w(F(n), "Invalid offset"), w(F(i), "Invalid length"), new t(r, n, i);
    }
    toString() {
    ...
    compare(t) {
        return this.chunkId < t.chunkId ? -1 : this.chunkId > t.chunkId ? 1 : this.offset < t.offset ? -1 : this.offset > t.offset ? 1 : (w(this.length === t.length), 0);
    }
    constructor(t, e, r) {
...
function tp(t, e, r) {
    ...
    t.read = function(t) {
        ...
        let r = O(e);
        if (t.writeUint8(r), !T(e)) switch (e.type) {
            case A.Array:
                return P(t, e);
            case A.Boolean:
                return _(t, e);
        ...
        switch (t.type) {
            case A.Array:
                return w(e.type === A.Array), q(t, e, r);
            case A.Boolean:
                return w(e.type === A.Boolean), D(t, e);
            case A.Color:
                return w(e.type === A.Color), J(t, e);
            case A.Date:
                return w(e.type === A.Date), z(t, e);
            case A.Enum:
                return w(e.type === A.Enum), H(t, e);
            case A.File:
                return w(e.type === A.File), Q(t, e);
            case A.Link:
                return w(e.type === A.Link), tt(t, e);
            case A.Number:
                return w(e.type === A.Number), tn(t, e);
            case A.Object:
                return w(e.type === A.Object), ta(t, e, r);
            case A.ResponsiveImage:
                return w(e.type === A.ResponsiveImage), tl(t, e);
            case A.RichText:
                return w(e.type === A.RichText), tf(t, e);
            case A.String:
                return w(e.type === A.String), tp(t, e, r);
            default:
                m(t);
        }
    };
...
var ty = class e {
        ...
        constructor(t, e) {
            this.fieldNames = t, this.options = e, this.entries = [];
        }
    },
    tv = 3,
    tw = 250,
    tm = [408, // Request Timeout
    ...
    tI = async (t, e) => {
        ...
        for (;;) {
            try {
                ...
                if (!tm.includes(n.status) || ++r > tv) return n;
            } catch (t) {
                if (e ? .signal ? .aborted || ++r > tv) throw t;
            }
            await tb(r);
        }
    };
async function tb(t) {
    let e = Math.floor(tw * (Math.random() + 1) * 2 ** (t - 1));
    await new Promise(t => {
        setTimeout(t, e);
    });
...
var tS = class {
    ...
    write(t, e) {
        ...
        for (; s > i; s--) {
            ...
            if (w(t, "Missing chunk"), !(n < t.start)) {
                if (n < t.end) {
                    let r = n - t.start,
                        i = t.data.subarray(r);
                    e = tk(e, i), n = t.end;
...
function tL(t) {
    w(t.length > 0, "Must have at least one range");
...
var tB = class {
        async loadModel() {
            let [t] = await tU(this.options.url, [this.options.range]);
            return w(t, "Failed to load model"), ty.deserialize(t);
        }
        async getModel() {
            return this.modelPromise ? ? = this.loadModel(), this.model ? ? = await this.modelPromise, this.model;
        }
        async lookupItems(t) {
            w(t.length === this.fields.length, "Invalid query length");
        ...
        queryContains(t, e, r) {
            return this.findItems(t, r, t => {
                if (t ? .type !== p.String || e.value ? .type !== p.String) return !1;
                let r = t.value,
                    n = e.value.value;
        ...
        constructor(t) {
            this.options = t, this.supportedLookupTypes = ["All" /* All */ , "Equals" /* Equals */ , "NotEquals" /* NotEquals */ , "LessThan" /* LessThan */ , "GreaterThan" /* GreaterThan */ , "Contains" /* Contains */ , "StartsWith" /* StartsWith */ , "EndsWith" /* EndsWith */ ], this.collation = this.options.collation;
            let e = {},
                r = [];
            for (let t of this.options.fieldNames) {
                let n = this.options.collectionSchema[t];
                w(n, "Missing definition for field", t), e[t] = n, r.push({
                    type: "Identifier",
                    name: t
                });
            }
    ...
    tM = class {
        scanItems() {
            return this.itemsPromise ? ? = tI(this.url).then(async t => {
        ...
        constructor(t, e) {
            this.id = t, this.url = e, this.itemLoader = new c.default(async t => {
                let e = t.map(t => {
                        let e = N.fromString(t);
                        return {
                            from: e.offset,
                            to: e.offset + e.length
                        };
                    }),
                    r = await tU(this.url, e);
                return r.map((e, r) => {
                    let n = new g(e),
                        i = tE.read(n),
                        s = t[r];
                    return w(s, "Missing pointer"), {
                        pointer: s,
                        data: i.getData()
                    };
                });
    ...
    tF = class {
        ...
        async resolveItems(t) {
            return Promise.all(t.map(t => {
                ...
                return w(r, "Missing chunk"), r.resolveItem(t);
            }));
        }
        compareItems(t, e) {
            let r = N.fromString(t.pointer),
                n = N.fromString(e.pointer);
            return r.compare(n);
        }
        compareValues(e, r, n) {
        ...
        constructor(t) {
            this.options = t, this.schema = this.options.schema, this.indexes = this.options.indexes, this.resolveRichText = this.options.resolveRichText, this.chunks = this.options.chunks.map((t, e) => new tM(e, t));
        }
    };
export {
    tF as DatabaseCollection, tB as DatabaseDictionaryIndex
};
